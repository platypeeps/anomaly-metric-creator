...
- follow-up: closed. The debug UI builds its unsupported-backlog CSV client-side (server_debug_ui.py `csvCell`), which never reaches the Python writer; 08-15-debug-ui-csv-formula-neutralization gave it the same guard, applied before quoting, and pinned the two trigger sets together with tools/check_csv_formula_trigger_lockstep.py. tests/test_debug_ui_javascript.py::test_debug_ui_csv_cell_neutralizes_formula_triggers (6 triggers through the served source under node), ::test_debug_ui_csv_cell_neutralizes_before_quoting, ::test_debug_ui_csv_cell_is_idempotent_and_spares_benign_cells, ::test_debug_ui_csv_cell_guard_is_present_without_node (node-independent floor); tests/test_csv_formula_trigger_lockstep_lint.py.
