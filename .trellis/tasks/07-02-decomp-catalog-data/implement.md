...
1. Monkeypatch grep for `MetricSpec`/`Instance`/`COMPONENTS`/`INSTANCES`
   /`DEFAULT_METRICS_PER_COMPONENT`/`DERIVATIONS` over `tests/`; runtime-
   reader audit for `_load_instance_config` (seam vs parameter per
   design rule).
2. Create `models_impl.py` then `catalog.py` (verbatim; validators move
   with registries at the same execution position); re-import blocks;
   splice-hazard grep.
3. Re-point `models.py` facade; run facade identity + registry tests;
   full suite; CLAUDE.md map; draft PR (`full-ci`) → checklist → merge.

**PR B (scenarios_impl + scenario_catalog + resolution cluster):**

4. Monkeypatch grep for `SCENARIOS` + resolution helpers; build
   `_configure_scenario_runtime` seam (legacy configures with lambdas).
5. Create `scenarios_impl.py` (builders, `Scenario`, spec validator,
...
6. `_EMIT_ARTIFACT_FILES` relabel/move housekeeping.
7. Re-point `scenarios.py` facade; record the scenario_catalog
   data-registry cap exemption in the PR + CLAUDE.md.
8. Full suite; `--help` byte-diff; draft PR (`full-ci`) → checklist →
