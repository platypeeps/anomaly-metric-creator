...
3. Split `test_matrix` into `test_heavy` + `test_light`. Identical setup
   ...
   uploading its `.coverage` via `actions/upload-artifact` with a distinct
   name. Pin the same action SHA already used at `ci.yml:405`.
4. Add the `coverage_combine` job: `needs: [test_heavy, test_light]`,
   ...
   then upload `coverage.xml` with `if: ${{ !cancelled() }}`.
...
- `coverage_combine` reports a percentage matching the pre-split figure;
