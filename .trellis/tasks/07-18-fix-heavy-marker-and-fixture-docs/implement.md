...
1. Branch from `main`. Record the baseline partition counts — every later
   ...
   ```
2. **Write the failing test first.** The fixture-definition audit failed on
   the live collision, reporting both `tests/test_schema_file.py` and
   `tests/test_validate_output.py` for `seven_day_schema_run`. This also
   exposed that isolated collection cannot resolve every module fixture via
   pytest's live fixture manager, so module definitions are inspected from
   source while session fixtures still use the manager.
...
4. Add the scoped collision guard: every registered heavy name must resolve
   to one definition. Module fixtures may live in test modules, so the guard
   forbids duplicate names rather than all out-of-conftest definitions:
   ```bash
...
6. Add `@pytest.mark.full_resolution` to
   ...
   ```
7. Correct the size figures to the setup-only measurements: 264 MiB / 4.12s
   for one-day N=3 and 1.81 GiB / 29.08s for seven-day N=3. State that they
   are on-disk output rather than resident memory. The stale `~5 GB`
   `pyproject.toml` comment was already absent.
...
`interval_seconds=1.0` on the renamed validator fixture, register that name
as heavy, re-run `pytest -m heavy --collect-only -q`, and confirm its two
consumers appear.
...
- `pyproject.toml` — document indirect parametrized lookup classification.
...
- Present the three fix options from `design.md` and why rename, scoped
  uniqueness, and collection-time callspec detection were combined.
