...
- [ ] `.venv/bin/pytest tests/test_server.py -n 0` green — this is the whole
...
- [ ] A round-trip test proves the replace path still writes every column —
...
- [ ] FTS assertions read the `command_traces_fts` table directly. A passing
      ...
      a false pass.
- [ ] Full suite + `pre-commit run --all-files` green; `check_mypy_gate.py`
      still clean at its 34 modules.
- [ ] `server_traces.py` is **not** one of those 34 modules
...
- [ ] This PR flips A-031 to `status: fixed` in `.trellis/audit/ledger.md`,
...
annotation inside the class body. The planned signature
(`conn: sqlite3.Connection`, `trace: CommandTrace`, `delete_fts_first: bool`)
avoids it. Do not "fix" the shadowing here — renaming a public store method is
