...
def scenario_deviation_results(amc, tmp_path_factory):
    ...
    is ``max_dev / base_std`` (or ``inf`` when the baseline column is
    constant); the acceptance check is ``ratio > 1.0``.
    """
    ...
    for slug, scenario in amc.SCENARIOS.items():
        ...
        with open(active_dir / "anomalies.csv") as fh:
        ...
        for r in rows:
            ...
            ts_a, vals_a, ts_b, vals_b = column_cache[key]
            ...
            ratio = dev / base_std if base_std > 0 else float("inf")
            per_scenario.append({
                "component": comp, "metric": metric,
                "ts": r["timestamp"], "span": (span_start, span_end),
