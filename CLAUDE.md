...
- Two-place version pins (`ruff==` in `pyproject.toml` ↔ `rev:` in `.pre-commit-config.yaml`) must be lint-enforced in lockstep (`check_ruff_lockstep.py`); a Dependabot bump must not silently raise a *declared* `>=` floor in `pyproject.toml` (use `versioning-strategy: lockfile-only`); and the Dependabot `package-ecosystem` must match a lockfile that actually exists (`uv` needs a committed `uv.lock`). Exact `==` pins that Dependabot cannot reach under `lockfile-only` (`mypy==` in `pyproject.toml`, `socketsecurity==` in `ci.yml`) have a manual update path — the "Pinned CI tool bumps" subsection of `docs/DEVELOPMENT_CYCLE.md` — so a bump does not silently rot.
