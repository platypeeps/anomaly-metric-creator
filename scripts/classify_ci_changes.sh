...
usage() {
  cat <<'EOF'
Usage: bash scripts/classify_ci_changes.sh [options] [changed-files.txt]
...
If changed-files.txt is omitted, the script compares HEAD to the merge base of
TRELLIS_CI_BASE_REF (default: origin/main) and includes unstaged/staged paths.
EOF
}
...
is_review_tooling_path() {
  case "$1" in
    scripts/classify_ci_changes.sh|scripts/trellis-full-check.sh|scripts/trellis-housekeeping.sh)
      return 0
      ;;
...
main() {
  ...
  if [ -n "$CHANGED_FILE_LIST" ]; then
    source_cmd=(cat "$CHANGED_FILE_LIST")
  else
    source_cmd=(collect_changed_files)
  fi
