...
| `--config` | _off_ | Optional JSON/YAML file containing `server` and `generate` defaults for `amc serve`. Explicit CLI flags override config values. Both sections are validated at load time and a bad key is rejected with the config path named: `server` keys against an allowlist, `generate` keys by parsing the config-derived flags through the real generate parser, so a typo like `componentss` fails immediately instead of surfacing later without file attribution. A `generate` key with a `null` value is also rejected, since it would otherwise produce no flag and vanish silently. |
