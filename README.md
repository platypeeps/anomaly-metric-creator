...
| `--config` | _off_ | Optional JSON/YAML file containing `server` and `generate` defaults for `amc serve`. Explicit CLI flags override config values. Both sections are validated at load time and a bad key is rejected with the config path named: `server` keys against an allowlist, `generate` keys by parsing the config-derived flags through the real generate parser, so a typo like `componentss` fails immediately instead of surfacing later without file attribution. A `generate` key whose value is `null` or `false` produces no flag at all, so the parser is asked about the bare flag instead: a real switch such as `otel_verbose: false` keeps its meaning of "use the default", while a typo, or a key that takes a value, is rejected rather than vanishing silently. |
