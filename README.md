...
| `--config` | _off_ | Optional JSON/YAML file containing `server` and `generate` defaults for `amc serve`. Explicit CLI flags override config values. Both sections are checked at load time — keys and values — so a mistake names the config file instead of surfacing later as an unattributed parser error. `server` keys check against an allowlist and `server` values are parsed by the real serve parser; `generate` keys and values are parsed by the real generate parser, so a typo like `componentss` fails immediately. `null` and `false` are the two values that produce no flag for either parser to see, so the key is checked on its own instead: a real switch such as `otel_verbose: false` keeps its meaning of "use the default", while a typo or a key that needs a value is rejected rather than vanishing silently. |
