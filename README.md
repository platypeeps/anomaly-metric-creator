...
| Flag | Default | Notes |
| ---- | ------- | ----- |
| `--version`         | —           | Print the installed distribution version and exit. An uninstalled source tree reports `0+unknown`. |
...
| `--config` | _off_ | Optional JSON/YAML file containing `server` and `generate` defaults for `amc serve`. Explicit CLI flags override config values. The file is checked at load against the real parsers — keys and the values the run will actually use — so that a config which would break the parse fails immediately, naming the file, instead of surfacing later as an unattributed parser error. A typo like `componentss` fails at once. Cross-flag rules that live outside the parsers (such as `--cors-allow-origin '*'` requiring `--auth-token`) are still checked at startup, not at load. A refusal names the file and the flags involved but never a value from the config, since any of them may be a secret; run the `serve` or `generate` command directly with those flags to see the parser's own message. `null` and `false` produce no flag for a parser to see, so the key is checked on its own instead — a real switch such as `otel_verbose: false` keeps its meaning of "use the default", while a typo or a key that needs a value is rejected rather than vanishing silently. |
...
amc serve --mcp-eval-mode --auth-token "$EVAL_TOKEN" \
  --persist-command-db eval-traces.sqlite \
  --duration-days 3 --scenarios cache_leak_restart \
  --components apigateway,cacheservice,database,mqservice
```

**Persist the command traces — in eval mode this is the harness's only way to
recover them.** The `/v1/debug/*` surfaces (including
...
- **`--inject-dst-artifact-day` is separated from gauge streaming by an intentional design boundary.**
...
| `mq_jam` | medium | 1 | 12:30 | 15 min DLQ blow-up + 20 min queue jam | `mqservice`, `apigateway`, `database`, `authservice` | Message queue DLQ blow-up + 1M pending + error rate 25%; cascades to slow API response, DB connection buildup, slow writes, auth session write delay. |
...
| `llm_viral_surge_day2` | medium | 2 | Day 2 10:15 | 12 min viral surge | `llm_analytics`, `database`, `cacheservice`, `apigateway` | Viral LLM surge — 8× request spike to 360/s, token surge 185k/s; cascades to gateway RPS, cache misses, DB query spike and connections. |
| `llm_enterprise_onboarding` | medium | 3 | Day 3 14:00 | 6h correlated capacity ramp | `llm_analytics`, `vectorstore`, `database`, `cacheservice` | Enterprise onboarding — requests, context windows, token-limit hits, embeddings, and LLM latency rise together; cascades to DB latency and cache memory. |
...
| `llm_weekend_batch` | medium | 6 | Day 6 02:00 | 4h correlated batch saturation | `llm_analytics`, `objectstore`, `database`, `cacheservice` | Weekend batch analytics — input tokens, context overflow, LLM latency, object-store bandwidth, and DB query/CPU pressure rise together; cascades to cache misses. |
| `llm_second_viral` | medium | 7 | Day 7 16:45 | 15 min viral surge | `llm_analytics`, `apigateway`, `database`, `cacheservice` | Second viral event — 10× spike to 450/s, 420k tokens/s; cascades to gateway active connections, CPU, DB connections, cache errors. |
...
| `regional_failover_storm` | **high** | 1 | 05:00 | 5 min | `loadbalancer`, `apigateway`, `database`, `authservice`, `mqservice` | Regional failover — backend 5xx ramps to 220/s over 5 min; cascades to gateway 5xx (~30%), DB connections (~9,000), auth errors (~40%), MQ pending ~500k. |
...
- **`test-resource-cost` (`tools/check_test_resource_cost.py`).** Rejects
  executable whole-file reads that can duplicate the memory cost of generated
  CSVs. Stream or use `sha256_path`; a reviewed, deliberately small artifact
  may use a trailing `# resource-lint: allow` marker.
