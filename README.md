...
| `--config` | _off_ | Optional JSON/YAML file containing `server` and `generate` defaults for `amc serve`. Explicit CLI flags override config values. The file is checked at load so that a config which would break the run fails immediately, naming the file, instead of surfacing later as an unattributed parser error: keys are checked against the real parsers, and so are the values the run will actually use. A typo like `componentss` fails at once. `null` and `false` produce no flag for a parser to see, so the key is checked on its own instead — a real switch such as `otel_verbose: false` keeps its meaning of "use the default", while a typo or a key that needs a value is rejected rather than vanishing silently. |
