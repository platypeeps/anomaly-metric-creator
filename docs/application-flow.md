...
flowchart TD
    ...
    torder -- "realistic (default since VER-156)" --> realorder["_topology_generation_order<br/>(Kahn's algorithm, topological order)<br/>+ _compose_topology_coupled_specs<br/>+ _compose_topology_saturation_specs<br/>+ dtype='int' columns rounded via np.rint"]
