...
* ``scripts/classify_ci_changes.sh`` owns path classification.
...
* CodeQL, Socket, and Dependabot workflows follow the same review-economy
  policy.
* ``scripts/trellis-full-check.sh`` mirrors the local quick/full gate.
...
def _check_ci(path: Path, text: str, violations: list[str]) -> None:
    for label, needle in [
        ...
        (
            ...
            "bash scripts/classify_ci_changes.sh --github-output changed-files.txt",
        ),
...
def _check_dependabot(path: Path, text: str, violations: list[str]) -> None:
    for label, needle in [
        ...
        ("approval step", "gh pr review --approve"),
...
def _check_classifier(path: Path, text: str, violations: list[str]) -> None:
    for label, needle in [
        ...
        ("review tooling scripts", "scripts/trellis-full-check.sh"),
    ]:
        _require_contains(text, needle, path=path, label=label, violations=violations)
...
def _check_full_check(path: Path, text: str, violations: list[str]) -> None:
    for label, needle in [
        ("quick/full level", "TRELLIS_FULL_CHECK_LEVEL"),
        ("contract guard", "tools/check_ci_review_contract.py"),
...
def check(root: Path) -> tuple[int, list[str]]:
    ...
    _check_docs(root / REQUIRED_FILES["review_pack"], texts["review_pack"], violations)
